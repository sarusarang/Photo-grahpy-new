...
export const ChangeBannerModal: React.FC<ChangeBannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overlay-animate overflow-y-auto">
      <div className="relative w-full max-w-3xl rounded-3xl bg-[#111218] border border-neutral-800 text-white shadow-2xl p-6 sm:p-8 my-8 sheet-animate">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="relative w-full h-44 sm:h-52 rounded-2xl overflow-hidden mb-6 border border-neutral-800 shadow-xl group">
          <img
          ...
          <div className="space-y-3">
            {event.media.length === 0 ? (
              <div className="py-12 text-center text-neutral-500 font-mono text-xs">
                No photos uploaded to this event yet. Upload photos first or choose a curated preset below.
              </div>
            ) : (
        ...
        {activeTab === 'upload' && (
          <div className="space-y-4">
            <label className="border-2 border-dashed border-neutral-800 hover:border-amber-400/50 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all bg-neutral-950/50 group">
              <Upload className="w-8 h-8 text-neutral-500 group-hover:text-amber-400 transition-colors mb-2" />
              <span className="text-sm font-semibold text-white">Click to upload banner image</span>
            ...
            <div className="space-y-2">
              <label className="text-xs font-mono text-neutral-400">Or Paste Image URL</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-neutral-900 border border-neutral-800 text-xs font-mono text-white focus:outline-none focus:border-amber-400"
                />
                <button
                  type="button"
                  onClick={() => {
        ...
        <div className="mt-6 pt-4 border-t border-neutral-800 flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-neutral-300 text-xs font-mono transition-colors cursor-pointer"
          >
            Cancel
          </button>
        </div>
