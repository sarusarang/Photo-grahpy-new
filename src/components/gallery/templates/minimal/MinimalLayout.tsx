...
export const MinimalLayout: React.FC<TemplateLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F2] text-neutral-900 font-sans selection:bg-neutral-900 selection:text-white">
      ...
      {/* ─── ARCHIVAL FILTER BAR ─── */}
      <div className="sticky top-0 z-30 bg-[#F5F5F2]/95 backdrop-blur-md border-b border-neutral-200/90 py-3 shadow-xs">
              ...
              return (
                <div key={item.id} className="group flex flex-col break-inside-avoid mb-3">
                  ...
                  >
                    ...
                    <div
                      onClick={() => onOpenLightbox(originalIndex)}
                      className="absolute inset-0 bg-black/15 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer pointer-events-none"
                    />
                      ...
                      <button
                        ...
                        className={`absolute bottom-2 right-2 z-10 p-1.5 rounded-full backdrop-blur-sm transition-all ${
                          isFav
                            ...
                            : 'bg-white/80 text-neutral-700 hover:bg-rose-500 hover:text-white opacity-0 group-hover:opacity-100 shadow-sm'
                        }`}
                  ...
                  {/* Caption below image */}
                  <div className="mt-1.5 flex items-baseline justify-between text-[10px] font-mono text-neutral-400 px-0.5">
                    ...
                    <span className="text-[9px] tracking-widest text-neutral-400 uppercase">
                      {item.sectionTitle || 'Print'}
                    </span>
                  </div>
                </div>
              );
